...
// what happens when spending goes over maximum?
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                if (data.category) {
                    newTotals[data.category] = (newTotals[data.category] || 0) + data.amount;
                }
            });
                    ...
                    <thead>
                        <tr className="font-bold bg-neutral-400 dark:bg-neutral-700 dark:text-neutral-100">
                            ...
                            <th scope="col" className="p-2 md:p-6 text-left text-sm md:text-lg">
                                <span className="hidden md:inline">Amount</span> Spent
                            </th>
                    ...
                    <tbody>
                        {categories.map((category, index) => (
                            <tr className="font-medium p-4 bg-neutral-300 hover:bg-neutral-500 dark:bg-neutral-800 dark:hover:bg-neutral-600 dark:text-neutral-300 rounded-bl-lg rounded-br-lg" key={index}>
                                ...
                                <td className="p-2 md:py-4 md:px-6 text-xs md:text-base">${categoryTotals[category.name] ? categoryTotals[category.name].toFixed(2) : 0}</td>
